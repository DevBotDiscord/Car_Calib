...
- Trong project này, scope ưu tiên của user chỉ xoay quanh steering commands và web route dashboard.
- Không tự ý sửa core vision pipeline, calibration algorithm, PID/calibration math, offline video processing, hoặc evaluation flow nếu không được yêu cầu rõ.
- Chỉ chạm vào calibration/core pipeline khi nó là nguyên nhân trực tiếp chặn steering commands hoặc web route dashboard; nếu cần chạm, giải thích ngắn gọn trước khi sửa.
