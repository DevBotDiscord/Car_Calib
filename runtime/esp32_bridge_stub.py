...
class ESP32SerialBridge:
    ...
    def diagnostics(self) -> dict[str, Any]:
        """Return lightweight bridge state for dashboard/debug endpoints."""
        return {
            "running": self._running,
            "connected": self.is_connected(),
            "port": self.current_port(),
            "scan_globs": list(self._port_globs),
    ...
    def device_kind(self) -> str | None:
        """Return handshake-detected device kind (esp32/esp8266), if known."""
            ...
            try:
                ...
                if port is None:
                    ...
                    if (
                        ...
                        self._running = False
                        return
                    self._last_scan = f"no handshake on {','.join(self._port_globs)}"
                    logger.warning(
                ...
                self._last_scan = f"connected on {port}"
    ...
    def _scan_and_handshake(self) -> str | None:
        ports = _scan_ports(self._port_globs)
        self._last_scan = f"ports={ports!r}"
        for port in ports:
            try:
            ...
            try:
                ...
                while time.monotonic() < deadline:
                    ...
                    if not line:
                        continue
                    if any(token in line for token in HANDSHAKE_TOKENS):
                        self._device_kind = "esp8266" if "CARCALIB-ESP8266" in line else "esp32"
                        with self._ser_lock:
                            self._ser = ser
                        return port
