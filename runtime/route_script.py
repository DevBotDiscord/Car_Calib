...
* ``forward``  / ``straight`` — base FORWARD, servo left to vision PID
* ``backward``                 — base BACKWARD, servo reversed from last script steer
...
_MOVING_BASE_COMMANDS = {"FORWARD", "BACKWARD", "TURN_LEFT", "TURN_RIGHT"}
_IDLE_BASE_COMMANDS = {"STOP", "LOCK", "UNLOCK"}
...
BASE_IDLE_POWER_ON_INTERVAL_S = _env_float("BASE_IDLE_POWER_ON_INTERVAL_S", 180.0)
BASE_IDLE_POWER_ON_ENABLED = os.getenv("BASE_IDLE_POWER_ON_ENABLED", "true").strip().lower() in {"1", "true", "t", "yes", "y", "on"}
...
class RouteScriptRunner:
    ...
    def __init__(self) -> None:
        ...
        self._last_script_steer_angle = SCRIPT_CENTER_ANGLE
        self._idle_power_stop = threading.Event()
        self._idle_power_thread: threading.Thread | None = None
        if BASE_IDLE_POWER_ON_ENABLED and BASE_IDLE_POWER_ON_INTERVAL_S > 0:
            self._idle_power_thread = threading.Thread(
                target=self._idle_power_loop,
                daemon=True,
                name="base-idle-power-on",
            )
            self._idle_power_thread.start()
    ...
    def _on_mqtt_connect(self, client: Any, _userdata: Any, _flags: Any, rc: int) -> None:
        if rc != 0:
            logger.warning("RouteScriptRunner MQTT connect failed rc=%s", rc)
            return
        try:
            client.subscribe(MQTT_BASE_COMMAND_TOPIC, qos=1)
            logger.info("RouteScriptRunner watching base commands on %s", MQTT_BASE_COMMAND_TOPIC)
        except Exception as exc:  # noqa: BLE001
            logger.error("RouteScriptRunner base subscribe failed: %s", exc)

    def _on_mqtt_message(self, _client: Any, _userdata: Any, msg: Any) -> None:
        if msg.topic != MQTT_BASE_COMMAND_TOPIC:
            return
        try:
            command = msg.payload.decode("utf-8", "replace").strip().upper()
        except Exception:  # noqa: BLE001
            return
        self._mark_base_command(command)

    def _mark_base_command(self, command: str) -> None:
        if command not in _MOVING_BASE_COMMANDS and command not in _IDLE_BASE_COMMANDS:
            return
        with self._base_state_lock:
            self._last_base_command = command
            self._last_base_command_at = time.monotonic()

    def _base_idle_for_s(self, now: float) -> float | None:
        with self._base_state_lock:
            if self._last_base_command in _MOVING_BASE_COMMANDS:
                return None
            return max(0.0, now - self._last_base_command_at)

    def _idle_power_loop(self) -> None:
        next_fire_at = time.monotonic() + BASE_IDLE_POWER_ON_INTERVAL_S
        while not self._idle_power_stop.wait(timeout=1.0):
            now = time.monotonic()
            if self.is_running():
                next_fire_at = now + BASE_IDLE_POWER_ON_INTERVAL_S
                continue
            idle_for_s = self._base_idle_for_s(now)
            if idle_for_s is None:
                next_fire_at = now + BASE_IDLE_POWER_ON_INTERVAL_S
                continue
            if idle_for_s < BASE_IDLE_POWER_ON_INTERVAL_S:
                next_fire_at = now + (BASE_IDLE_POWER_ON_INTERVAL_S - idle_for_s)
                continue
            if now >= next_fire_at:
                logger.info("Base idle %.0fs; publishing POWER ON pulse", idle_for_s)
                self.publish_power(True)
                next_fire_at = now + BASE_IDLE_POWER_ON_INTERVAL_S

    def close(self) -> None:
        self.stop()
        self._idle_power_stop.set()
        if self._thread is not None:
            self._thread.join(timeout=2.0)
        if self._idle_power_thread is not None:
            self._idle_power_thread.join(timeout=2.0)
        try:
    ...
    def _reversed_script_angle(self, angle: float) -> float:
        return SCRIPT_CENTER_ANGLE - (angle - SCRIPT_CENTER_ANGLE)
    ...
    def _execute_step(self, step: dict[str, Any]) -> None:
        ...
        if action in ("forward", "straight"):
            ...
            base_cmd = "BACKWARD"
            angle = self._reversed_script_angle(self._last_script_steer_angle)
        elif action == "left":
            base_cmd = "FORWARD"
        ...
        if action in ("left", "right") and angle is not None:
            self._last_script_steer_angle = angle
        ...
        try:
            ...
            if action in ("left", "right", "backward"):
